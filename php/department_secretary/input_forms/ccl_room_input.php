...
$sql = "SELECT MAX(id) AS last_id FROM tbl_room"; // Assuming your primary key is 'id'
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "add") {
        ...
        $check_sql = "SELECT * FROM tbl_room WHERE room_code=? AND dept_code=?";
        $check_stmt = $conn->prepare($check_sql);
        if ($check_stmt === false) {
            $message = "Error preparing statement: " . $conn->error;
        } else {
            $check_stmt->bind_param("ss", $room_code, $dept_code);
            $check_stmt->execute();
        ...
        $stmt->bind_param("sssssss", $dept_code, $room_code, $room_name, $room_type, $room_in_charge, $room_id, $status);
        ...
        if ($stmt->execute()) {
            ...
            foreach ($tables as $table) {
                ...
                $check_sql = "SELECT COUNT(*) FROM $table WHERE room_code = ? AND semester = ? AND ay_code = ?";
                $check_stmt = $conn->prepare($check_sql);
                $check_stmt->bind_param("sss", $old_room_code, $semester, $ay_code);
                $check_stmt->execute();
        ...
        foreach ($shared_tables as $table) {
            ...
            if ($sched_count > 0) {
                ...
                echo "<script>
                    document.addEventListener('DOMContentLoaded', function() {
                        var modal = new bootstrap.Modal(document.getElementById('successModal'));
                        document.getElementById('successMessage').textContent = 'There's a plotted schedule in $room_code. You can't delete the record.';
                        modal.show();
                    });
                </script>";
...
<body>
    ...
    <section class="class-input">
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="room-tab" href="ccl_room_input.php" aria-controls="room" aria-selected="true">Room Input</a>
            </li>
            <li class="nav-item">
        ...
        <div class="row">
            <div class="col-lg-4 mb-4">
                ...
                <form action="" method="POST" id="room-form" required>
                    ...
                    <?php
                    // Check if the session contains the college code
                    $college_code = $_SESSION['college_code'] ?? '';

                    if ($college_code) {
                        // Fetch department names based on the college code
                        $query = "SELECT dept_code FROM tbl_department WHERE college_code = ?";
                    ...
                    <div class="mb-3">
                        <select class="form-control" id="dept_code" name="dept_code" style="color: #6c757d;" required>
                            <option value="" disabled selected>Select Room Location</option>
                            <?php foreach ($departments as $department): ?>
                                <option value="<?php echo htmlspecialchars($department['dept_code'], ENT_QUOTES, 'UTF-8'); ?>">
                                    <?php echo htmlspecialchars($department['dept_code'], ENT_QUOTES, 'UTF-8'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-3 ">
                        ...
                        <input type="text" class="form-control" id="room_code" placeholder="Enter Room Code"
                            name="room_code" autocomplete="off" style="color: #6c757d;" required>
                    </div>

                    <div class="mb-3">
                        <!-- <label for="room_name">Room Name</label> -->
                        <input type="text" class="form-control" id="room_name" placeholder="Enter Room Name"
                            name="room_name" autocomplete="off" style="color: #6c757d;" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="room_in_charge" placeholder="Enter Room In Charge"
                            name="room_in_charge" autocomplete="off" style="color: #6c757d;" required>
                    </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                          ...
                          // Initialize variables
                            ...
                            // Base SQL query
                            $sql = "SELECT * FROM tbl_room WHERE room_type IN ('Computer Laboratory')";

                            // Add conditions based on search filters
                            if (!empty($search_status)) {
                                $sql .= " AND status = ?";
                                $params[] = $search_status; // Add parameter value
                                $types .= "s"; // Add type for bind_param
                            }

                            if (!empty($search_room_code_name)) {
                                $sql .= " AND (room_code LIKE ? OR room_name LIKE ? OR room_in_charge LIKE ?)";
                                $params[] = "%$search_room_code_name%"; // Add parameter for room_code
                                $params[] = "%$search_room_code_name%"; // Add parameter for room_name
                                $params[] = "%$search_room_code_name%"; // Add parameter for room_in_charge
                                $types .= "sss"; // Add types for bind_param
                            }
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr 
                                        ...
                                        data-dept_code='" . htmlspecialchars($row['dept_code']) . "'
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fillForm(roomId, deptCode, roomCode, roomName, roomInCharge, roomStatus) {
            ...
            document.querySelectorAll('table tbody tr').forEach(row => {
                row.addEventListener('click', function() {
                    const roomId = this.getAttribute('data-room_id'); // Get room_id from data attribute
                    const cells = this.getElementsByTagName('td');
                    fillForm(roomId, cells[0].innerText, cells[1].innerText, cells[2].innerText, cells[3].innerText, cells[4].innerText);
            ...
            document.addEventListener('click', function(event) {
                ...
                const userType = document.getElementById('user_type').value; // Get user_type

                // If clicked outside the form and table
                if (!roomForm.contains(event.target) && !table.contains(event.target)) {
                    if (selectedRow) {
                        // Clear the values inside the form but keep the fields visible
