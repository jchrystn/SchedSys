...
<body>
    ...
    <body>
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item">
                <a class="nav-link active" id="professor-tab" href="../report/majorsub_summary.php" aria-controls="professor"
            ...
            <div class="search-bar-container">
                <form method="POST" action="majorsub_summary.php" class="row">
                    <div class="col-md-3">
                    </div>
                    <div class="col-md-3">
            ...
            <div class="container" id="scheduleContent">
                ...
                <div class="card-body schedule-content">
                    ...
                    $sql_courses = "SELECT course_code, course_name FROM tbl_course WHERE course_type = 'Major'";
            ...
            WHERE ts.course_code = ? AND ts.semester = ?";
                            ...
                            if ($result_schedule->num_rows > 0) {
                                while ($row_schedule = $result_schedule->fetch_assoc()) {
                                    ...
                                    if (!isset($major_schedule_summary[$program_code][$unique_key])) {
                                        // Store the entry only if it doesn't exist
                                        $major_schedule_summary[$program_code][$unique_key] = $schedule_entry;
                                    } else {
                                        // If it already exists, we update it to include the new room, professor, and schedule details
                                        $existing_entry = &$major_schedule_summary[$program_code][$unique_key];
                    ...
                    function displayScheduleTable($schedule_summary, $program_code, $semester, $ay_code)
                    {
                        if (!empty($schedule_summary)) {
                            ...
                            // Table with hidden data attributes for semester and academic year
                    ...
                    // Loop through major schedule summary and pass semester & ay_code dynamically
                    foreach ($major_schedule_summary as $program_code => $schedule_summary) {
                        $semester = "1st_Semester";  // Fetch this dynamically if available
                        $ay_code = "2024-2025";      // Fetch this dynamically if available
                        displayScheduleTable($schedule_summary, $program_code, $semester, $ay_code);
                    }
                    
                    if (empty($major_schedule_summary)) {
