    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($prof_code !== "TBA") {
                ...
                // Delete the schedule from the professor's schedule table
                $sql_delete_prof = "DELETE FROM $sanitized_prof_sched_code WHERE sec_sched_id = ? AND semester = ? AND dept_code = ?";
                $stmt_delete_prof = $conn->prepare($sql_delete_prof);
                $stmt_delete_prof->bind_param('sss', $sec_sched_id, $semester, $dept_code);
                ...
                $fetch_prof_hours_query = "SELECT * FROM tbl_psched_counter WHERE prof_sched_code = '$prof_sched_code' and semester='$semester' AND dept_code = '$dept_code'";
                ...
                if ($check_result->num_rows === 0) {
                    ...
                    $update_prep_hours_query = "UPDATE tbl_psched_counter SET prep_hrs = '$prep_hours' WHERE prof_sched_code = '$prof_sched_code' AND semester = '$semester' AND dept_code = '$dept_code'";
                ...
                // Store receiver_dept_codes in an array
                $receiver_dept_codes = [];
                ...
                echo "Sender Dept Code: $dept_code<br>";
                echo "Recevier Dept Code: $receiver_dept_code<br><br>";
                ...
                if (!empty($receiver_dept_codes)) {
                    foreach ($receiver_dept_codes as $receiver_dept_code) {
                        ...
                        // If records exist, delete and update teaching hours
                        if ($row_count_shared > 0) {
                            ...
                            if ($total_hours > 0) {
                                ...
                                $sql_delete_shared = "DELETE FROM $sanitized_receiver_prof_sched_code WHERE sec_sched_id = ? AND semester = ? AND dept_code = ?";
            ...
            if ($sanitized_section_sched_code) {
                $sql_shared_sched = "SELECT sender_dept_code FROM tbl_shared_sched WHERE receiver_dept_code = ?";
                $stmt_shared_sched = $conn->prepare($sql_shared_sched);
                $stmt_shared_sched->bind_param('s', $receiver_dept_code);
                ...
                if ($sender_dept_code) {
                    ...
                    echo "sanitized sender (sanitized_sender_section_sched_code): $sanitized_sender_section_sched_code<br><br>";
                    ...
                    if ($row_count_shared > 0) {
                        // Delete the shared schedule if it exists
                        $sql_delete_shared = "DELETE FROM $sanitized_sender_section_sched_code WHERE sec_sched_id = ? AND semester = ?";
                        $stmt_delete_shared = $conn->prepare($sql_delete_shared);
                        $stmt_delete_shared->bind_param('ss', $sec_sched_id, $semester);
                        if (!$stmt_delete_shared->execute()) {
                            echo "Error deleting from shared section's schedule table: " . $stmt_delete_shared->error;
                        }
                        $stmt_delete_shared->close();
                    }
                    ...
                    // No records in tbl_shared_sched, proceed with the regular section schedule deletion
...
$sql = "
    ...
    AND tbl_rsched.room_type = 'Computer Laboratory'
...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_schedule' && isset($_GET['room_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<h5>Schedule for Classroom: " . htmlspecialchars($room_id) . "</h5>";
...
function fetchScheduleForRoom($room_sched_code, $ay_code, $semester)
{
    ...
    $sql_fetch_room_info = "SELECT room_code, ay_code, dept_code FROM tbl_rsched WHERE room_sched_code=? AND dept_code=? AND ay_code=?";
    ...
    try {
        // Fetch schedule data for the room
        $sql_fetch_schedule = "SELECT * FROM $sanitized_room_sched_code WHERE semester = ? AND room_sched_code = ? AND dept_code = ?";
        $stmt_schedule = $conn->prepare($sql_fetch_schedule);
        $stmt_schedule->bind_param("sss", $semester, $room_sched_code, $dept_code);
        ...
        if ($result_schedule->num_rows > 0) {
            ...
            while ($row_schedule = $result_schedule->fetch_assoc()) {
                ...
                $section_code = $result_section->num_rows > 0 ? $result_section->fetch_assoc()['section_code'] : '';

                // Fetch cell color from tbl_schedstatus
        ...
        return '<p>Error fetching schedule: ' . htmlspecialchars($e->getMessage()) . '</p>';
    }
}
                ...
                foreach ($schedule_data[$day_name] as $index => $schedule) {
                    ...
                    if ($schedule_start == $current_start) {
                        ...
                        $cell_content = "<div style='text-align: center; font-size: 12px; vertical-align: center;  ;'>
                         <span style='font-weight: bold;'>{$schedule['course_code']}<br> {$schedule['class_type']}</span><br>";

                        $cell_content .= "{$section_code}<br>{$schedule['prof_name']}</div>";
...
<body>
    ...
    <div class="header">
        <h2 class="title"> <i class="fa-solid fa-bars-progress" style="color:#FD7238;"></i> COMPUTER LABORATORY</h2>
    ...
    <div class="container mt-8">
            ...
            <div class="row mb-4">
                <div class="col-md-12">
                    <div class="search-bar-container">
                        <form method="POST" action="lib_comlab.php" class="row">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <input type="text" class="form-control"  name="search_room" value="<?php echo isset($_POST['search_room']) ? htmlspecialchars($_POST['search_room']) : ''; ?>" placeholder="Search" autocomplete="off">
                            </div>
                            <div class="col-md-3">
                                    ...
                                    <tr data-room-id="<?php echo htmlspecialchars($row['room_code']); ?>">
                                        <td style="width: 80px;">
                                            <div style="display: flex;">
                                                <form method="POST" action="lib_comlab.php">
                                                    <input type="hidden" name="room_code" value="<?php echo htmlspecialchars($row['room_code'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                    <input type="hidden" name="semester" value="<?php echo htmlspecialchars($row['semester'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                    <input type="hidden" name="ay_code" value="<?php echo htmlspecialchars($row['ay_code'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                    <input type="hidden" name="status" value="<?php echo htmlspecialchars($row['status'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                    <input type="hidden" name="room_sched_code" value="<?php echo htmlspecialchars($row['room_sched_code'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                    ...
                                                    <button type="button" class="change-btn lock-btn"
                                                        ...
                                                        <?php echo ($currentStatus == 'public') ? 'style="display:none;"' : ''; ?>>
                                        ...
                                        <td>
                                            <!-- Delete Form -->
                                            <form method="POST" action="lib_comlab.php" style="display:inline;">
                                                <input type="hidden" name="room_code" value="<?php echo htmlspecialchars($row['room_code']); ?>">
                                                ...
                                                <input type="hidden" name="room_sched_code" value="<?php echo htmlspecialchars($row['room_sched_code']); ?>">
                                                <button type="submit" name="action" value="delete" class="delete-btn" onclick="return confirm('Are you sure you want to delete this record?')">
        ...
        <div class="modal fade" id="privateModal" tabindex="-1" aria-labelledby="privateModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="privateModalLabel">Success</h5>
                    </div>
                    <div class="modal-body">
                ...
                $('#makePublicButton').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            window.location.href = 'lib_comlab.php';
                        },
                        error: function() {
